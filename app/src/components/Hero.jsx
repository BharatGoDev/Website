
export default function Example() {
  return (
    <div className="bg-black lg:px-10 lg:pt-20 lg:pr-40 lg:pb-40 lg:pl-40">
      <div className="flex items-center">
        <img src="/hero.svg" alt="hero" className="lg:w-80 lg:h-80 lg:mt-0 hidden sm:hidden md:hidden lg:block" />
        <div className="sm:text-center md:text-center text-right pl-20 ml-20 ">

          <h1 className="text-4xl font-bold tracking-tight sm:text-6xl  ">
            <span style={{ color: "#FF9933" }} >Bhar</span>
            <span style={{ color: "#FFFFFF" }}>atG</span>
            <span style={{ color: "#138808" }}>oDev</span>
          </h1>

          <p>
            <div className="text-center pt-2">
              <span style={{ color: "#FFFFFF" }}>
                Web & Graphic Design Specialists
              </span>
            </div>
          </p>

        </div>
      </div>
    </div>
  );
}
